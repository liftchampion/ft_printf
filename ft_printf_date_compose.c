...
int ft_add_year(t_date *dt, long int *n)
{
	if (*n < (IS_LEAP_YEAR(dt->year) + 365) * DAY)
		return (0);
...
int ft_add_month(t_date *dt, long int *n)
{
	if (*n < (months[dt->mth] + (dt->mth == 1 && IS_LEAP_YEAR(dt->year))) * DAY)
		return (0);
	*n -= (months[dt->mth] + (dt->mth == 1 && IS_LEAP_YEAR(dt->year))) * DAY;
...
int ft_add_day(t_date *dt, long int *n)
{
	if (*n < DAY)
		return (0);
	*n -= DAY;
	if (dt->day == months[dt->mth] + (dt->mth == 1 && IS_LEAP_YEAR(dt->year))
														&& (dt->day = 1))
		if (dt->mth == 11 && !(dt->mth = 0))
			++dt->year;
		else
...
int ft_add_hour(t_date *dt, long int *n)
{
	...
	*n -= 3600;
	if (dt->hr == 23 && !(dt->hr = 0))
		if (dt->day == months[dt->mth] + (dt->mth == 1 &&
...
int		ft_printf_date_compose(t_arg_data *ad, void* arg, t_string **str)
{
	...
	*dt = (t_date){1970, 0, 1, 0, 0, 0};
	while (ft_add_year(dt, &n))
		;
	while (ft_add_month(dt, &n))
		;
	while (ft_add_day(dt, &n))
		;
	while (ft_add_hour(dt, &n))
		;
	while (ft_add_minute(dt, &n))
		;
	while (ft_add_second(dt, &n))
		;
